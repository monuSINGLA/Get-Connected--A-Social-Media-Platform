...
const Post = ({ post }) => {
  ...
  const { mutate: deletePost, isPending: isDeleting } = useMutation({
    mutationFn: async () => {
      try {
        ...
        if (!res.ok)
          throw new Error(data.error || "Unable to delete post right now");
      } catch (error) {
  ...
  const { mutate: likePost, isPending: isLiking } = useMutation({
    ...
    onSuccess: (updatedLikes) => {
      //this is not the best way bcoz it will refatch all post and realod the ui
      // queryClient.invalidateQueries({queryKey: ["posts"]})

      //instead of refacthing all post update the cache directly for that post
            ...
            <div className="flex gap-4 items-center w-2/3 justify-between">
              ...
              >
                <div className="modal-box rounded border border-gray-600">
                  ...
                  <div
                    className="scrollDesign flex flex-col gap-3 max-h-60 overflow-auto"
